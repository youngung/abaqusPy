...
c     Using  dE = dE^(el)^(k+1) + dlamb^(k+1),
         ...
         if (idiaw) then
            ...
            call w_dim(idia,dstran_pl_ks(k,:),ntens,1d0,.true.)
            call w_chr(idia,'dstran_pl_k+1')
            call w_dim(idia,dstran_pl_ks(k+1,:),ntens,1d0,.true.)
            call w_empty_lines(idia,2)
            call w_chr(idia,'dstran_el_k')
            call w_dim(idia,dstran_el_ks(k,:),ntens,1d0,.true.)
            call w_chr(idia,'dstran_el_k+1')
            call w_dim(idia,dstran_el_ks(k+1,:),ntens,1d0,.true.)
            call w_empty_lines(idia,2)
            call w_chr(idia,' stran_pl_k')
            call w_dim(idia, stran_pl_ks(k,:),ntens,1d0,.true.)
            call w_chr(idia,' stran_pl_k+1')
            call w_dim(idia, stran_pl_ks(k+1,:),ntens,1d0,.true.)
            call w_empty_lines(idia,2)
            call w_chr(idia,' stran_el_k')
            call w_dim(idia, stran_el_ks(k,:),ntens,1d0,.true.)
            call w_chr(idia,' stran_el_k+1')
            call w_dim(idia, stran_el_ks(k+1,:),ntens,1d0,.true.)
            call w_chr(idia,' old predictor stress [MPa]')
            call w_dim(idia, spr_ks(k,:),ntens,1/empa,.true.)
            call w_chr(idia,' new predictor stress [MPa]')
            call w_dim(idia, spr_ks(k+1,:),ntens,1/empa,.true.)
         endif
