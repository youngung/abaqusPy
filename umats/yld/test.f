...
c     to test various yield functions
      ...
      integer nth,nyldc
      parameter(nth=5)
      dimension yldc(9),rs(nth),ys(nth)
      real*8 yldc,rs,ys
      real*8 toler
...
c-----------------------------------------------------------------------n
      ...
      implicit none
      integer nth,i,j,ii,jj,iyld,nyldc
     ...
     $     dphi33ld(3,3),yldc(nyldc),aux33(3,3),
     $     aux3(3),bux3(3),rs,ys,rv
      logical verbose
      pi=4d0*datan(1d0)
...
c-----------------------------------------------------------------------
     ...
     $     'e11_l','e22_l','e33_l','e12_l','|'
      do 10 j=1,nth
         ...
         if (verbose) write(*,'(f7.2)',advance='no') th*180.d0/pi
c$$$
c$$$  si_lab
c$$$
         if (verbose) write(*,'(4f7.2,x,a1,x)',advance='no')
...
c$$$
         ...
         rs(j)=rv
         if (verbose) write(*,'(4f7.2,x,a1,x,2f7.2)',advance='no')
